...
app.get('/api/admin/premium', requireAuth, requireInstanceAdmin, (_req, res) => {
  ...
  return res.json({
    enabled: instanceSetting('premiumEnabled', 'true') === 'true', freeForAll: instanceSetting('premiumFreeForAll', 'false') === 'true', name: instanceSetting('premiumName', 'Open+'),
...
app.put('/api/admin/premium', requireAuth, requireInstanceAdmin, (req: AuthenticatedRequest, res) => {
  const parsed = z.object({
    enabled: z.boolean(), freeForAll: z.boolean().optional().default(false), name: z.string().trim().min(1).max(40), description: z.string().max(240), color: hexColorSchema,
  ...
  setInstanceSetting('premiumFreeForAll', String(parsed.data.freeForAll));
