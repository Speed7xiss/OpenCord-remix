...
export function isServerOwner(userId: number, serverId: number) {
  return Boolean(db.prepare('SELECT 1 FROM servers WHERE id = ? AND owner_id = ?').get(serverId, userId));
}

export function getServerPermissions(userId: number, serverId: number) {
  if (isServerOwner(userId, serverId)) return ALL_PERMISSIONS;
  ...
  `).all(userId, serverId) as Array<{ permissions: number }>;
  return rows.reduce((permissions, row) => permissions | Number(row.permissions ?? 0), 0);
}

export function hasPermission(userId: number, serverId: number, permission: number) {
...
function applyChannelOverwrites(base: number, channelId: number, serverId: number, userId: number) {
  let permissions = base;
  const everyone = db.prepare('SELECT id FROM roles WHERE server_id = ? AND is_everyone = 1').get(serverId) as { id: number } | undefined;
  if (everyone) {
    const overwrite = db.prepare("SELECT allow_permissions, deny_permissions FROM channel_permission_overwrites WHERE channel_id = ? AND target_type = 'ROLE' AND target_id = ?")
      .get(channelId, everyone.id) as any;
    if (overwrite) permissions = (permissions & ~Number(overwrite.deny_permissions)) | Number(overwrite.allow_permissions);
  }

  const roles = db.prepare('SELECT role_id FROM member_roles WHERE server_id = ? AND user_id = ?').all(serverId, userId) as Array<{ role_id: number }>;
  let roleAllow = 0;
  let roleDeny = 0;
  for (const role of roles) {
    const overwrite = db.prepare("SELECT allow_permissions, deny_permissions FROM channel_permission_overwrites WHERE channel_id = ? AND target_type = 'ROLE' AND target_id = ?")
      .get(channelId, role.role_id) as any;
    if (overwrite) {
  ...
  if (memberOverwrite) permissions = (permissions & ~Number(memberOverwrite.deny_permissions)) | Number(memberOverwrite.allow_permissions);
  return permissions;
}

export function getChannelPermissions(userId: number, channelId: number) {
  const channel = db.prepare('SELECT id, server_id, parent_id, kind FROM channels WHERE id = ?').get(channelId) as { id: number; server_id: number; parent_id: number | null; kind: string } | undefined;
  if (!channel) return 0;
  ...
  if (channel.parent_id) {
    const parent = db.prepare("SELECT id FROM channels WHERE id = ? AND server_id = ? AND kind = 'CATEGORY'").get(channel.parent_id, serverId) as { id: number } | undefined;
    if (parent) permissions = applyChannelOverwrites(permissions, Number(parent.id), serverId, userId);
  }
...
export function hasChannelPermission(userId: number, channelId: number, permission: number) {
