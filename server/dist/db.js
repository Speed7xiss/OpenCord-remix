...
function migration(version, run) {
...
migration(4, () => {
  ...
  `);
    ...
    for (const [name, imagePath] of defaults)
        update.run(imagePath, name);
});
migration(5, () => {
...
export function activePremiumForUser(userId, now = Date.now()) {
...
export function publicUser(row, includePrivate = false) {
...
export function activeInstanceBan(userId, now = Date.now()) {
...
export function instanceAudit(actorId, action, targetType, targetId, details = {}) {
