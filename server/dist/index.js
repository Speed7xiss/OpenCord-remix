...
import { z } from 'zod';
import { clearSessionCookie, createSession, destroyAllSessions, destroySession, getUserFromToken, readSessionToken, requireAuth, requireInstanceAdmin, setSessionCookie, } from './auth.js';
import { audit } from './audit.js';
import { activeInstanceBan, activePremiumForUser, backupDir, createBackup, db, instanceAudit, premiumBenefits, publicUser, rootDir } from './db.js';
import { ALL_PERMISSIONS, DEFAULT_MEMBER_PERMISSIONS, getChannelPermissions, getServerPermissions, hasChannelPermission, hasPermission, isServerOwner, Permission, } from './permissions.js';
...
const configuredOrigins = new Set([process.env.PUBLIC_URL, ...(process.env.ALLOWED_ORIGINS ?? '').split(',')]
...
function isAllowedRequestOrigin(req, originHeader) {
    let origin;
    try {
    ...
    if (!['http:', 'https:'].includes(origin.protocol))
        return false;
    if (configuredOrigins.has(origin.origin))
        return true;
    const hostHeader = req.get('host');
    if (hostHeader && origin.host === hostHeader)
        return true;
    if (isDevelopment && hostHeader) {
        const requestHostname = hostHeader.startsWith('[')
            ? hostHeader.slice(1, hostHeader.indexOf(']'))
            : hostHeader.split(':')[0];
        if (origin.hostname === requestHostname)
            return true;
        if (isLoopbackHostname(origin.hostname) && isLoopbackHostname(requestHostname))
            return true;
    }
    return false;
}
...
function makeUpload(allowed, maxBytes) {
    return multer({
...
function premiumMembership(userId) {
    return activePremiumForUser(userId);
}
function hasPremiumBenefit(userId, key) {
...
function numericPremiumBenefit(userId, key, fallback) {
    const premium = premiumMembership(userId);
    if (!premium)
        return fallback;
    const value = premium.benefits[key];
...
function removeUserUpload(filePath) {
    if (filePath?.startsWith('/uploads/'))
        fs.rm(path.join(uploadDir, path.basename(filePath)), { force: true }, () => undefined);
}
function saveProfileHistory(userId) {
    const premium = premiumMembership(userId);
    ...
    avatar_decoration_path, profile_background_path, custom_join_sound_path, special_identity FROM users WHERE id = ?`).get(userId);
    if (!row)
        return;
...
function userById(id, includePrivate = false) {
...
function serverForChannel(channelId) {
    return db.prepare('SELECT server_id, kind FROM channels WHERE id = ?').get(channelId);
}
...
function emitServerRefresh(serverId) {
...
function messageAttachments(messageId) {
    return db.prepare('SELECT id, path, original_name, mime_type, size_bytes FROM message_attachments WHERE message_id = ? ORDER BY id').all(messageId).map((item) => ({
        id: Number(item.id), path: item.path, name: item.original_name, mimeType: item.mime_type, size: Number(item.size_bytes),
    }));
}
function messageReactions(messageId, viewerId) {
...
function messageRowToJson(row, viewerId) {
    let replyTo = null;
    if (row.reply_to_id) {
    ...
    `).get(row.reply_to_id);
        if (reply)
            replyTo = { id: Number(reply.id), content: reply.content, author: publicUser({ ...reply, id: reply.author_id, created_at: reply.user_created_at }) };
    }
    return {
        id: Number(row.id),
...
function getMessage(messageId, viewerId) {
...
function ensureEveryoneRole(serverId) {
    const existing = db.prepare('SELECT id FROM roles WHERE server_id = ? AND is_everyone = 1').get(serverId);
    if (existing)
...
function serverMemberRoles(serverId, userId) {
    return db.prepare(`
  ...
  `).all(userId, serverId, serverId).map((r) => ({
...
function publicPresence(userId, declared) {
    if (!onlineUsers.get(userId))
        return 'offline';
    return declared === 'invisible' ? 'offline' : declared;
}
function voiceStatesForServer(serverId) {
...
app.post('/api/auth/register', authLimiter, async (req, res) => {
    ...
    if (!parsed.success)
        return fail(res, 400, 'Invalid registration data.');
    const { username, password, displayName } = parsed.data;
    const isFirst = Number(db.prepare('SELECT COUNT(*) AS count FROM users').get().count) === 0;
    if (!isFirst && instanceSetting('registrationEnabled', 'true') !== 'true')
        return fail(res, 403, 'New registrations are disabled on this instance.');
    ...
    if (isFirst) {
        const adminBadge = db.prepare("SELECT id FROM badges WHERE name = 'Administrator'").get();
        if (adminBadge)
            db.prepare('INSERT OR IGNORE INTO user_badges (user_id, badge_id, assigned_by) VALUES (?, ?, ?)').run(userId, Number(adminBadge.id), userId);
    }
...
app.post('/api/auth/login', authLimiter, async (req, res) => {
    ...
    const row = db.prepare('SELECT * FROM users WHERE username = ?').get(parsed.data.username);
    if (!row || row.disabled || !(await bcrypt.compare(parsed.data.password, row.password_hash)))
...
app.patch('/api/me', requireAuth, (req, res) => {
    ...
    if (!parsed.success)
        return fail(res, 400, 'Invalid profile.');
    ...
    if ((parsed.data.bio?.length ?? 0) > bioMax)
        return fail(res, 400, `Bio can contain at most ${bioMax} characters.`);
    if ((parsed.data.statusText?.length ?? 0) > statusMax)
        return fail(res, 400, `Status can contain at most ${statusMax} characters.`);
    const currentRow = db.prepare('SELECT * FROM users WHERE id = ?').get(req.user.id);
    if (parsed.data.username && parsed.data.username.toLowerCase() !== String(currentRow.username).toLowerCase()) {
        if (db.prepare('SELECT 1 FROM users WHERE username = ? AND id <> ?').get(parsed.data.username, req.user.id))
            return fail(res, 409, 'That username is already in use.');
    }
    saveProfileHistory(req.user.id);
    db.prepare('UPDATE users SET username = ?, display_name = ?, bio = ?, status_text = ?, presence = ? WHERE id = ?').run(parsed.data.username ?? currentRow.username, parsed.data.displayName ?? currentRow.display_name, parsed.data.bio ?? currentRow.bio, parsed.data.statusText ?? currentRow.status_text, parsed.data.presence ?? currentRow.presence, req.user.id);
...
app.get('/api/me/premium', requireAuth, (req, res) => {
    const premium = premiumMembership(req.user.id);
    return res.json({
        premium,
        system: {
...
app.post('/api/me/premium/redeem', requireAuth, (req, res) => {
    if (instanceSetting('premiumEnabled', 'true') !== 'true')
        return fail(res, 403, 'Premium memberships are currently disabled on this instance.');
    const parsed = z.object({ code: z.string().trim().min(8).max(80) }).safeParse(req.body);
    if (!parsed.success)
        return fail(res, 400, 'Invalid redemption code.');
    const normalized = parsed.data.code.toUpperCase();
    const codeHash = crypto.createHash('sha256').update(normalized).digest('hex');
    try {
        const result = db.transaction(() => {
            const row = db.prepare('SELECT * FROM premium_redeem_codes WHERE code_hash = ?').get(codeHash);
            if (!row || row.disabled || Number(row.use_count) >= Number(row.max_uses))
                throw new Error('INVALID_CODE');
            if (db.prepare('SELECT 1 FROM premium_redeem_usage WHERE code_id = ? AND user_id = ?').get(Number(row.id), req.user.id))
                throw new Error('ALREADY_REDEEMED');
            const now = Date.now();
            const existing = db.prepare('SELECT starts_at, expires_at FROM user_premium WHERE user_id = ?').get(req.user.id);
            if (existing && existing.expires_at == null)
                throw new Error('ALREADY_PERMANENT');
            let expiresAt = null;
            if (row.duration_minutes != null)
                expiresAt = Math.max(now, Number(existing?.expires_at ?? 0)) + Number(row.duration_minutes) * 60_000;
            ...
            if (update.changes === 0)
                db.prepare('INSERT INTO user_premium (user_id, granted_by, starts_at, expires_at, updated_at) VALUES (?, NULL, ?, ?, CURRENT_TIMESTAMP)').run(req.user.id, now, expiresAt);
        ...
        if (error instanceof Error && error.message === 'INVALID_CODE')
            return fail(res, 404, 'This redemption code is invalid, disabled, or fully used.');
        if (error instanceof Error && error.message === 'ALREADY_REDEEMED')
            return fail(res, 409, 'You already redeemed this code.');
        if (error instanceof Error && error.message === 'ALREADY_PERMANENT')
            return fail(res, 409, 'Your account already has a permanent premium membership.');
        console.error('[Premium] Redemption failed:', error);
        return fail(res, 500, 'Could not redeem this code.');
    }
});
app.get('/api/me/profile-history', requireAuth, (req, res) => {
    if (!hasPremiumBenefit(req.user.id, 'profileHistoryDays'))
        return fail(res, 403, 'Your plan does not include profile history.');
    ...
    const rows = db.prepare('SELECT id, snapshot, created_at FROM user_profile_history WHERE user_id = ? AND created_at >= ? ORDER BY id DESC LIMIT 200')
        .all(req.user.id, Date.now() - days * 86400000);
    return res.json({ history: rows.map((row) => ({ id: Number(row.id), snapshot: safeJson(row.snapshot), createdAt: Number(row.created_at) })) });
});
app.patch('/api/me/premium-profile', requireAuth, (req, res) => {
    const premium = premiumMembership(req.user.id);
    if (!premium)
        return fail(res, 403, 'This feature requires the instance premium plan.');
    ...
    if (!parsed.success)
        return fail(res, 400, 'Invalid customization.');
    if (parsed.data.profileTheme && !premium.benefits.customProfileTheme)
        return fail(res, 403, 'Profile themes are not included in your plan.');
    if (parsed.data.profileGradient && !premium.benefits.profileGradient)
        return fail(res, 403, 'Profile gradients are not included in your plan.');
    if (parsed.data.profileEffect && !premium.benefits.profileEffects)
        return fail(res, 403, 'Profile effects are not included in your plan.');
    if (parsed.data.specialIdentity && !premium.benefits.specialIdentity)
        return fail(res, 403, 'Special identity is not included in your plan.');
    const current = db.prepare('SELECT * FROM users WHERE id = ?').get(req.user.id);
    saveProfileHistory(req.user.id);
    db.prepare('UPDATE users SET profile_theme = ?, profile_gradient = ?, profile_effect = ?, special_identity = ? WHERE id = ?').run(parsed.data.profileTheme ?? current.profile_theme, parsed.data.profileGradient ?? current.profile_gradient, parsed.data.profileEffect ?? current.profile_effect, parsed.data.specialIdentity ?? current.special_identity, req.user.id);
    const user = userById(req.user.id, true);
    io.emit('user:update', user);
    return res.json({ user });
});
app.post('/api/me/avatar', requireAuth, profileImageUpload.single('avatar'), (req, res) => {
    if (!req.file)
        return fail(res, 400, 'Select a PNG, JPEG, WebP, or GIF image.');
    if (req.file.mimetype === 'image/gif' && !hasPremiumBenefit(req.user.id, 'animatedAvatar')) {
        removeUploaded([req.file]);
        return fail(res, 403, 'Animated avatars require the premium plan.');
    }
    saveProfileHistory(req.user.id);
...
app.post('/api/me/banner', requireAuth, profileImageUpload.single('banner'), (req, res) => {
    if (!req.file)
        return fail(res, 400, 'Select a PNG, JPEG, WebP, or GIF image.');
    if (req.file.mimetype === 'image/gif' && !hasPremiumBenefit(req.user.id, 'animatedBanner')) {
        removeUploaded([req.file]);
        return fail(res, 403, 'Animated banners require the premium plan.');
    }
    saveProfileHistory(req.user.id);
    const nextPath = `/uploads/${req.file.filename}`;
    const old = db.prepare('SELECT banner_path FROM users WHERE id = ?').get(req.user.id);
...
app.post('/api/me/avatar-decoration', requireAuth, premiumImageUpload.single('image'), (req, res) => {
    if (!hasPremiumBenefit(req.user.id, 'avatarDecoration')) {
        if (req.file)
            removeUploaded([req.file]);
        return fail(res, 403, 'Avatar decorations are not included in your plan.');
    }
    if (!req.file)
        return fail(res, 400, 'Select an image.');
    const nextPath = `/uploads/${req.file.filename}`;
    const old = db.prepare('SELECT avatar_decoration_path FROM users WHERE id = ?').get(req.user.id);
...
app.post('/api/me/profile-background', requireAuth, premiumImageUpload.single('image'), (req, res) => {
    if (!hasPremiumBenefit(req.user.id, 'profileBackground')) {
        if (req.file)
            removeUploaded([req.file]);
        return fail(res, 403, 'Profile backgrounds are not included in your plan.');
    }
    if (!req.file)
        return fail(res, 400, 'Select an image.');
    const nextPath = `/uploads/${req.file.filename}`;
    const old = db.prepare('SELECT profile_background_path FROM users WHERE id = ?').get(req.user.id);
...
app.post('/api/me/custom-join-sound', requireAuth, premiumSoundUpload.single('sound'), (req, res) => {
    if (!hasPremiumBenefit(req.user.id, 'customJoinSound')) {
        if (req.file)
            removeUploaded([req.file]);
        return fail(res, 403, 'Custom join sounds are not included in your plan.');
    }
    if (!req.file)
        return fail(res, 400, 'Select an MP3, OGG, or WAV file.');
    const nextPath = `/uploads/${req.file.filename}`;
    const old = db.prepare('SELECT custom_join_sound_path FROM users WHERE id = ?').get(req.user.id);
...
function premiumEmojiToken(name, id) {
    return `<:${name}:${id}>`;
}
function premiumEmojiPayload(row) {
    return {
        id: Number(row.id),
...
function validateCustomEmojiContent(userId, content) {
    const tokens = [...content.matchAll(/<:([a-z0-9_]{2,32}):(\d+)>/gi)];
    if (!tokens.length)
        return null;
    if (!hasPremiumBenefit(userId, 'externalEmojis'))
        return 'Custom emojis are not included in your plan.';
    for (const token of tokens) {
        const row = db.prepare('SELECT id, name FROM premium_emojis WHERE id = ?').get(Number(token[2]));
        if (!row || String(row.name).toLowerCase() !== token[1].toLowerCase())
            return 'The message contains an invalid custom emoji.';
    }
    return null;
}
app.get('/api/emojis', requireAuth, (req, res) => {
    ...
    const rows = canUseExternal
        ? db.prepare(`SELECT * FROM premium_emojis WHERE (? = '' OR name LIKE ?) ORDER BY name COLLATE NOCASE LIMIT 120`).all(search, `%${search}%`)
        : db.prepare(`SELECT * FROM premium_emojis WHERE user_id = ? AND (? = '' OR name LIKE ?) ORDER BY name COLLATE NOCASE LIMIT 120`).all(req.user.id, search, `%${search}%`);
    const favorites = db.prepare('SELECT emoji_value FROM favorite_emojis WHERE user_id = ? ORDER BY created_at DESC').all(req.user.id).map((row) => String(row.emoji_value));
    return res.json({ emojis: rows.map(premiumEmojiPayload), favorites, favoriteLimit: premium?.benefits.favoriteEmojiSlots ?? 20 });
});
app.post('/api/me/emojis', requireAuth, premiumEmojiUpload.single('image'), (req, res) => {
    if (!hasPremiumBenefit(req.user.id, 'externalEmojis')) {
        if (req.file)
            removeUploaded([req.file]);
        return fail(res, 403, 'Custom emojis are not included in your plan.');
    }
    if (!req.file)
        return fail(res, 400, 'Select a PNG, JPEG, WebP, or GIF image.');
    ...
    if (!/^[a-z0-9_]{2,32}$/.test(name)) {
        removeUploaded([req.file]);
        return fail(res, 400, 'Use 2 to 32 characters: letters, numbers, and _.');
    }
    const count = Number(db.prepare('SELECT COUNT(*) AS count FROM premium_emojis WHERE user_id = ?').get(req.user.id).count);
    if (count >= 100) {
        removeUploaded([req.file]);
        return fail(res, 400, 'Limit of 100 custom emojis per user reached.');
    }
    try {
...
app.delete('/api/me/emojis/:emojiId', requireAuth, (req, res) => {
    const emojiId = parseId(req.params.emojiId);
    const row = emojiId ? db.prepare('SELECT * FROM premium_emojis WHERE id = ? AND user_id = ?').get(emojiId, req.user.id) : null;
    if (!row)
        return fail(res, 404, 'Emoji not found.');
...
app.put('/api/me/emoji-favorites', requireAuth, (req, res) => {
    const value = String(req.body?.value ?? '').trim().slice(0, 80);
    if (!value)
        return fail(res, 400, 'Invalid emoji.');
    const premium = premiumMembership(req.user.id);
    const limit = Math.max(1, Number(premium?.benefits.favoriteEmojiSlots ?? 20));
    const current = Number(db.prepare('SELECT COUNT(*) AS count FROM favorite_emojis WHERE user_id = ?').get(req.user.id).count);
    ...
    if (!exists && current >= limit)
        return fail(res, 400, `Favorite emoji limit of ${limit} reached.`);
    db.prepare('INSERT OR IGNORE INTO favorite_emojis (user_id, emoji_value) VALUES (?, ?)').run(req.user.id, value);
    return res.status(204).end();
});
app.delete('/api/me/emoji-favorites', requireAuth, (req, res) => {
    const value = String(req.body?.value ?? '').trim().slice(0, 80);
    if (!value)
        return fail(res, 400, 'Invalid emoji.');
...
app.get('/api/emojis/:emojiId/image', requireAuth, (req, res) => {
    const emojiId = parseId(req.params.emojiId);
    const row = emojiId ? db.prepare('SELECT image_path FROM premium_emojis WHERE id = ?').get(emojiId) : null;
    if (!row?.image_path)
        return fail(res, 404, 'Emoji not found.');
...
app.get('/api/users/search', requireAuth, (req, res) => {
  ...
  `).all(req.user.id, `%${q}%`, `%${q}%`);
    return res.json({ users: rows.map((row) => ({ ...publicUser(row), presence: publicPresence(Number(row.id), row.presence) })) });
});
app.get('/api/users/:userId/profile', requireAuth, (req, res) => {
    const userId = parseId(req.params.userId);
    if (!userId)
        return fail(res, 404, 'User not found.');
    const row = db.prepare('SELECT * FROM users WHERE id = ? AND disabled = 0').get(userId);
    if (!row)
        return fail(res, 404, 'User not found.');
  ...
  `).get(req.user.id, req.user.id, req.user.id, userId, userId).count);
  ...
  `).all(req.user.id, userId).map((s) => ({ id: Number(s.id), name: s.name, iconPath: s.icon_path ?? null }));
    return res.json({
...
app.put('/api/blocks/:userId', requireAuth, (req, res) => {
    const targetId = parseId(req.params.userId);
    if (!targetId || targetId === req.user.id || !userById(targetId))
...
app.get('/api/friends', requireAuth, (req, res) => {
    ...
    const decorate = (row) => ({ friendshipId: Number(row.friendship_id), unreadCount: Number(row.unread_count ?? 0), user: { ...publicUser(row), presence: publicPresence(Number(row.id), row.presence) } });
  ...
  `).all(uid, uid, uid, uid, uid).map(decorate);
    ...
    const blocked = db.prepare(`SELECT u.* FROM blocked_users b JOIN users u ON u.id = b.blocked_id WHERE b.blocker_id = ?`).all(uid).map((row) => publicUser(row));
...
app.post('/api/friends/request/:userId', requireAuth, (req, res) => {
    ...
    if (!targetId || targetId === req.user.id || !userById(targetId))
        return fail(res, 400, 'Invalid user.');
    if (isBlocked(req.user.id, targetId))
        return fail(res, 403, 'You cannot send a friend request to this user.');
    const existing = friendshipStatus(req.user.id, targetId);
    if (existing)
        return fail(res, 409, existing.status === 'accepted' ? 'You are already friends.' : 'A pending friend request already exists.');
    db.prepare("INSERT INTO friendships (requester_id, addressee_id, status) VALUES (?, ?, 'pending')").run(req.user.id, targetId);
    io.to(`user:${targetId}`).emit('friends:refresh');
    return res.status(201).json({ ok: true });
});
app.post('/api/friends/:friendshipId/accept', requireAuth, (req, res) => {
    const friendshipId = parseId(req.params.friendshipId);
    if (!friendshipId)
        return fail(res, 400, 'Invalid friend request.');
    const row = db.prepare("SELECT * FROM friendships WHERE id = ? AND addressee_id = ? AND status = 'pending'").get(friendshipId, req.user.id);
    if (!row)
        return fail(res, 404, 'Friend request not found.');
    if (isBlocked(Number(row.requester_id), Number(row.addressee_id)))
        return fail(res, 403, 'Friend request blocked.');
...
app.delete('/api/friends/:friendshipId', requireAuth, (req, res) => {
    const friendshipId = parseId(req.params.friendshipId);
    if (!friendshipId)
        return fail(res, 400, 'Invalid friendship.');
    const row = db.prepare('SELECT * FROM friendships WHERE id = ? AND (requester_id = ? OR addressee_id = ?)').get(friendshipId, req.user.id, req.user.id);
    if (!row)
...
app.post('/api/servers', requireAuth, (req, res) => {
    ...
    if (!parsed.success)
        return fail(res, 400, 'Invalid server name.');
    ...
    const ownedServers = Number(db.prepare('SELECT COUNT(*) AS c FROM servers WHERE owner_id = ?').get(req.user.id).c);
    if (maxServers > 0 && ownedServers >= maxServers)
        return fail(res, 403, `Limit of ${maxServers} servers per user reached.`);
...
app.post('/api/servers/join', requireAuth, (req, res) => {
    ...
    const invite = db.prepare(`SELECT i.*, s.id AS sid FROM server_invites i JOIN servers s ON s.id = i.server_id WHERE i.code = ?`).get(code)
        ?? db.prepare('SELECT id AS server_id, id AS sid, NULL AS expires_at, NULL AS max_uses, 0 AS uses FROM servers WHERE invite_code = ?').get(code);
    if (!invite)
        return fail(res, 404, 'Invalid invite.');
    const serverId = Number(invite.server_id ?? invite.sid);
    if (invite.expires_at && Number(invite.expires_at) <= Date.now())
        return fail(res, 410, 'This invite has expired.');
    if (invite.max_uses && Number(invite.uses) >= Number(invite.max_uses))
        return fail(res, 410, 'This invite has reached its usage limit.');
    if (db.prepare('SELECT 1 FROM server_bans WHERE server_id = ? AND user_id = ?').get(serverId, req.user.id))
        return fail(res, 403, 'You are banned from this server.');
    const inserted = db.prepare('INSERT OR IGNORE INTO server_members (server_id, user_id) VALUES (?, ?)').run(serverId, req.user.id);
    if (inserted.changes && invite.code)
        db.prepare('UPDATE server_invites SET uses = uses + 1 WHERE code = ?').run(code);
    audit(serverId, req.user.id, 'MEMBER_JOIN', 'USER', req.user.id);
    emitServerRefresh(serverId);
    return res.json({ serverId });
});
app.get('/api/servers/:serverId', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !isMember(req.user.id, serverId))
        return fail(res, 404, 'Server not found.');
    ensureEveryoneRole(serverId);
    const serverRow = db.prepare('SELECT * FROM servers WHERE id = ?').get(serverId);
    const channels = db.prepare('SELECT * FROM channels WHERE server_id = ? ORDER BY position, id').all(serverId)
        ...
        overwrites: hasPermission(req.user.id, serverId, Permission.MANAGE_ROLES)
            ? db.prepare('SELECT target_type, target_id, allow_permissions, deny_permissions FROM channel_permission_overwrites WHERE channel_id = ?').all(Number(c.id)).map((o) => ({ targetType: o.target_type, targetId: Number(o.target_id), allow: Number(o.allow_permissions), deny: Number(o.deny_permissions) }))
            : [],
    }));
  ...
  `).all(serverId).map((m) => {
    ...
    const roles = db.prepare('SELECT * FROM roles WHERE server_id = ? ORDER BY position DESC, id').all(serverId).map((r) => ({
        id: Number(r.id), name: r.name, color: r.color, permissions: Number(r.permissions), position: Number(r.position), isEveryone: Boolean(r.is_everyone),
    }));
    return res.json({
        server: {
...
app.patch('/api/servers/:serverId', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !hasPermission(req.user.id, serverId, Permission.MANAGE_SERVER))
    ...
    if (!parsed.success)
        return fail(res, 400, 'Invalid configuration.');
    const current = db.prepare('SELECT name, description FROM servers WHERE id = ?').get(serverId);
...
app.post('/api/servers/:serverId/icon', requireAuth, imageUpload.single('icon'), (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !hasPermission(req.user.id, serverId, Permission.MANAGE_SERVER)) {
        if (req.file)
            removeUploaded([req.file]);
    ...
    const nextPath = `/uploads/${req.file.filename}`;
    const old = db.prepare('SELECT icon_path FROM servers WHERE id = ?').get(serverId);
    db.prepare('UPDATE servers SET icon_path = ? WHERE id = ?').run(nextPath, serverId);
    if (old?.icon_path?.startsWith('/uploads/'))
        fs.rm(path.join(uploadDir, path.basename(old.icon_path)), { force: true }, () => undefined);
    audit(serverId, req.user.id, 'SERVER_ICON_UPDATE', 'SERVER', serverId);
    emitServerRefresh(serverId);
    return res.json({ iconPath: nextPath });
});
app.post('/api/servers/:serverId/banner', requireAuth, imageUpload.single('banner'), (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !hasPermission(req.user.id, serverId, Permission.MANAGE_SERVER)) {
        if (req.file)
            removeUploaded([req.file]);
    ...
    if (!req.file)
        return fail(res, 400, 'Invalid image.');
    const nextPath = `/uploads/${req.file.filename}`;
    const old = db.prepare('SELECT banner_path FROM servers WHERE id = ?').get(serverId);
    db.prepare('UPDATE servers SET banner_path = ? WHERE id = ?').run(nextPath, serverId);
    if (old?.banner_path?.startsWith('/uploads/'))
        fs.rm(path.join(uploadDir, path.basename(old.banner_path)), { force: true }, () => undefined);
    audit(serverId, req.user.id, 'SERVER_BANNER_UPDATE', 'SERVER', serverId);
    emitServerRefresh(serverId);
    return res.json({ bannerPath: nextPath });
});
app.put('/api/servers/:serverId/me-profile', requireAuth, premiumImageUpload.single('avatar'), (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !isMember(req.user.id, serverId)) {
        if (req.file)
            removeUploaded([req.file]);
    ...
    if (!hasPremiumBenefit(req.user.id, 'perServerProfiles')) {
        if (req.file)
            removeUploaded([req.file]);
        return fail(res, 403, 'Per-server profiles are not included in your plan.');
    }
    ...
    if (!parsed.success) {
        if (req.file)
            removeUploaded([req.file]);
        return fail(res, 400, 'Invalid server profile.');
    }
    const current = db.prepare('SELECT * FROM server_profiles WHERE server_id = ? AND user_id = ?').get(serverId, req.user.id);
    ...
    if (req.file && current?.avatar_path && current.avatar_path !== nextAvatar)
        removeUserUpload(current.avatar_path);
    emitServerRefresh(serverId);
    return res.json({ ok: true });
});
app.post('/api/servers/:serverId/leave', requireAuth, (req, res) => {
    ...
    if (!serverId || !isMember(req.user.id, serverId))
        return fail(res, 404, 'Server not found.');
    if (isServerOwner(req.user.id, serverId))
        return fail(res, 409, 'Transfer ownership or delete the server before leaving.');
...
app.delete('/api/servers/:serverId', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !isServerOwner(req.user.id, serverId))
        return fail(res, 403, 'Only the owner can delete the server.');
    db.prepare('DELETE FROM servers WHERE id = ?').run(serverId);
    io.emit('server:deleted', { serverId });
    return res.status(204).end();
});
app.post('/api/servers/:serverId/transfer', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    const userId = parseId(req.body?.userId);
    if (!serverId || !userId || !isServerOwner(req.user.id, serverId) || !isMember(userId, serverId))
        return fail(res, 400, 'Invalid ownership transfer.');
...
app.get('/api/servers/:serverId/invites', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !hasPermission(req.user.id, serverId, Permission.MANAGE_SERVER))
        return fail(res, 403, 'Permission denied.');
    const invites = db.prepare('SELECT * FROM server_invites WHERE server_id = ? ORDER BY created_at DESC').all(serverId).map((i) => ({
...
app.post('/api/servers/:serverId/invites', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !isMember(req.user.id, serverId))
        return fail(res, 403, 'Permission denied.');
    ...
    if (!parsed.success)
        return fail(res, 400, 'Invalid invite configuration.');
...
app.delete('/api/servers/:serverId/invites/:code', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !hasPermission(req.user.id, serverId, Permission.MANAGE_SERVER))
        return fail(res, 403, 'Permission denied.');
...
app.post('/api/servers/:serverId/channels', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !hasPermission(req.user.id, serverId, Permission.MANAGE_CHANNELS))
        return fail(res, 403, 'Permission denied.');
    const parsed = z.object({
    ...
    const position = Number(db.prepare('SELECT COALESCE(MAX(position), -1) AS max FROM channels WHERE server_id = ?').get(serverId).max) + 1;
    const result = db.prepare('INSERT INTO channels (server_id, name, kind, parent_id, position, topic, user_limit, bitrate) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(serverId, parsed.data.name, parsed.data.kind, parsed.data.kind === 'CATEGORY' ? null : (parsed.data.parentId ?? null), position, parsed.data.topic ?? '', parsed.data.userLimit ?? 0, parsed.data.bitrate ?? 64000);
...
app.patch('/api/channels/:channelId', requireAuth, (req, res) => {
    ...
    const row = channelId ? db.prepare('SELECT * FROM channels WHERE id = ?').get(channelId) : null;
    if (!row || !hasPermission(req.user.id, Number(row.server_id), Permission.MANAGE_CHANNELS))
        return fail(res, 403, 'Permission denied.');
    const parsed = z.object({
    ...
    if (parsed.data.parentId) {
        ...
        if (!parent || Number(parsed.data.parentId) === channelId)
            return fail(res, 400, 'Invalid category.');
    }
    db.prepare('UPDATE channels SET name = ?, parent_id = ?, topic = ?, user_limit = ?, bitrate = ?, position = ? WHERE id = ?').run(parsed.data.name ?? row.name, row.kind === 'CATEGORY' ? null : (parsed.data.parentId !== undefined ? parsed.data.parentId : row.parent_id), parsed.data.topic ?? row.topic, parsed.data.userLimit ?? row.user_limit, parsed.data.bitrate ?? row.bitrate, parsed.data.position ?? row.position ?? 0, channelId);
...
app.post('/api/servers/:serverId/channels/reorder', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !hasPermission(req.user.id, serverId, Permission.MANAGE_CHANNELS))
        return fail(res, 403, 'Permission denied.');
    ...
    if (!parsed.success)
        return fail(res, 400, 'Invalid ordering.');
    ...
    db.transaction(() => { for (const item of parsed.data.items)
        update.run(item.position, item.parentId ?? null, item.id, serverId); })();
...
app.put('/api/channels/:channelId/permissions', requireAuth, (req, res) => {
    const channelId = parseId(req.params.channelId);
    const row = channelId ? serverForChannel(channelId) : null;
    if (!row || !hasPermission(req.user.id, Number(row.server_id), Permission.MANAGE_ROLES))
        return fail(res, 403, 'Permission denied.');
    ...
    if (!parsed.success)
        return fail(res, 400, 'Invalid permissions.');
...
app.get('/api/channels/:channelId/messages', requireAuth, (req, res) => {
    const channelId = parseId(req.params.channelId);
    if (!channelId || !canAccessChannel(req.user.id, channelId))
        return fail(res, 403, 'You do not have access to this channel.');
    ...
    const rows = (before
        ...
        : db.prepare(`SELECT m.*, u.username, u.display_name, u.bio, u.status_text, u.avatar_path, u.banner_path, u.presence, u.created_at AS user_created_at FROM messages m JOIN users u ON u.id = m.author_id WHERE m.channel_id = ? ORDER BY m.id DESC LIMIT ?`).all(channelId, limit));
    const messages = rows.reverse().map((row) => messageRowToJson(row, req.user.id));
    return res.json({ messages, hasMore: rows.length === limit });
});
app.get('/api/channels/:channelId/messages/search', requireAuth, (req, res) => {
    const channelId = parseId(req.params.channelId);
    const q = String(req.query.q ?? '').trim();
    if (!channelId || !canAccessChannel(req.user.id, channelId))
        return fail(res, 403, 'You do not have access to this channel.');
    ...
    FROM messages m JOIN users u ON u.id = m.author_id WHERE m.channel_id = ? AND m.content LIKE ? ORDER BY m.id DESC LIMIT 50`).all(channelId, `%${q}%`);
    return res.json({ messages: rows.map((row) => messageRowToJson(row, req.user.id)) });
});
app.get('/api/channels/:channelId/pins', requireAuth, (req, res) => {
    const channelId = parseId(req.params.channelId);
    if (!channelId || !canAccessChannel(req.user.id, channelId))
        return fail(res, 403, 'You do not have access to this channel.');
    const rows = db.prepare(`SELECT m.*, u.username, u.display_name, u.bio, u.status_text, u.avatar_path, u.banner_path, u.presence, u.created_at AS user_created_at
    FROM messages m JOIN users u ON u.id = m.author_id WHERE m.channel_id = ? AND m.pinned = 1 ORDER BY m.id DESC LIMIT 100`).all(channelId);
    return res.json({ messages: rows.map((row) => messageRowToJson(row, req.user.id)) });
});
app.post('/api/channels/:channelId/messages', requireAuth, attachmentUpload.array('files', 10), (req, res) => {
    ...
    if (files.length > maxFiles) {
        ...
        return fail(res, 413, `Your limit is ${maxFiles} files per message.`);
    }
    const maxUploadBytes = maxUploadMb * 1024 * 1024;
    if (files.some((file) => file.size > maxUploadBytes)) {
        removeUploaded(files);
        return fail(res, 413, `Your per-file limit is ${Math.round(maxUploadBytes / 1024 / 1024)} MB.`);
    }
    const channelId = parseId(req.params.channelId);
    if (!channelId || !canAccessChannel(req.user.id, channelId) || !hasChannelPermission(req.user.id, channelId, Permission.SEND_MESSAGES)) {
        removeUploaded(files);
        return fail(res, 403, 'You do not have permission to send messages.');
    }
    ...
    if (emojiError) {
        removeUploaded(files);
        return fail(res, 403, emojiError);
    }
    ...
    if (replyToId) {
        ...
        if (!reply) {
            removeUploaded(files);
            return fail(res, 400, 'Invalid replied message.');
        }
    }
    const messageId = db.transaction(() => {
        ...
        for (const file of files)
            insertAttachment.run(id, `/uploads/${file.filename}`, path.basename(file.originalname).slice(0, 160), file.mimetype, file.size);
        return id;
    })();
    ...
    WHERE sm.server_id = ? AND u.id <> ? AND u.disabled = 0`).all(serverId, req.user.id);
    for (const recipient of recipients) {
        if (recipient.muted || recipient.level === 'NONE')
            continue;
        const mentioned = content.includes(`@${recipient.username}`) || content.includes('@everyone');
        if (recipient.level === 'MENTIONS' && !mentioned)
            continue;
...
app.patch('/api/messages/:messageId', requireAuth, (req, res) => {
    ...
    if (emojiError)
        return fail(res, 403, emojiError);
    const row = db.prepare('SELECT * FROM messages WHERE id = ? AND author_id = ?').get(messageId, req.user.id);
    if (!row)
...
app.delete('/api/messages/:messageId', requireAuth, (req, res) => {
    ...
    if (!messageId)
        return fail(res, 400, 'Invalid message.');
    const row = db.prepare(`SELECT m.*, c.server_id FROM messages m JOIN channels c ON c.id = m.channel_id WHERE m.id = ?`).get(messageId);
    if (!row)
        return fail(res, 404, 'Message not found.');
    const allowed = Number(row.author_id) === req.user.id || hasPermission(req.user.id, Number(row.server_id), Permission.MANAGE_MESSAGES);
    if (!allowed)
        return fail(res, 403, 'Permission denied.');
    const attachments = db.prepare('SELECT path FROM message_attachments WHERE message_id = ?').all(messageId);
    db.prepare('DELETE FROM messages WHERE id = ?').run(messageId);
    for (const attachment of attachments)
        if (attachment.path.startsWith('/uploads/'))
            fs.rm(path.join(uploadDir, path.basename(attachment.path)), { force: true }, () => undefined);
...
app.put('/api/messages/:messageId/pin', requireAuth, (req, res) => {
    const messageId = parseId(req.params.messageId);
    const row = messageId ? db.prepare('SELECT m.channel_id, c.server_id FROM messages m JOIN channels c ON c.id = m.channel_id WHERE m.id = ?').get(messageId) : null;
    if (!row || !hasPermission(req.user.id, Number(row.server_id), Permission.MANAGE_MESSAGES))
        return fail(res, 403, 'Permission denied.');
...
app.delete('/api/messages/:messageId/pin', requireAuth, (req, res) => {
    const messageId = parseId(req.params.messageId);
    const row = messageId ? db.prepare('SELECT m.channel_id, c.server_id FROM messages m JOIN channels c ON c.id = m.channel_id WHERE m.id = ?').get(messageId) : null;
    if (!row || !hasPermission(req.user.id, Number(row.server_id), Permission.MANAGE_MESSAGES))
        return fail(res, 403, 'Permission denied.');
...
app.put('/api/messages/:messageId/reactions/:emoji', requireAuth, (req, res) => {
    const messageId = parseId(req.params.messageId);
    const emoji = String(req.params.emoji ?? '').slice(0, 80);
    const row = messageId ? db.prepare('SELECT channel_id FROM messages WHERE id = ?').get(messageId) : null;
    if (!row || !canAccessChannel(req.user.id, Number(row.channel_id)) || !emoji)
        return fail(res, 400, 'Invalid reaction.');
    const custom = /^<:([a-z0-9_]{2,32}):(\d+)>$/i.exec(emoji);
    if (custom) {
        const customRow = db.prepare('SELECT id, user_id, name FROM premium_emojis WHERE id = ?').get(Number(custom[2]));
        if (!customRow || String(customRow.name).toLowerCase() !== custom[1].toLowerCase())
            return fail(res, 400, 'Invalid custom emoji.');
        if (Number(customRow.user_id) !== req.user.id && !hasPremiumBenefit(req.user.id, 'externalReactions'))
            return fail(res, 403, 'Custom emoji reactions are not included in your plan.');
    }
...
app.delete('/api/messages/:messageId/reactions/:emoji', requireAuth, (req, res) => {
    const messageId = parseId(req.params.messageId);
    const emoji = String(req.params.emoji ?? '').slice(0, 80);
    const row = messageId ? db.prepare('SELECT channel_id FROM messages WHERE id = ?').get(messageId) : null;
    if (!row)
...
app.post('/api/channels/:channelId/read', requireAuth, (req, res) => {
    ...
    const last = db.prepare('SELECT COALESCE(MAX(id), 0) AS id FROM messages WHERE channel_id = ?').get(channelId);
...
app.get('/api/dms/:userId', requireAuth, (req, res) => {
    ...
    if (!otherId || !userById(otherId))
        return fail(res, 404, 'User not found.');
    const friendship = friendshipStatus(req.user.id, otherId);
    if (!friendship || friendship.status !== 'accepted' || isBlocked(req.user.id, otherId))
        return fail(res, 403, 'Direct messages are available only between unblocked friends.');
    const before = parseId(req.query.before);
...
app.post('/api/dms/:userId', requireAuth, (req, res) => {
    ...
    const friendship = friendshipStatus(req.user.id, otherId);
    if (!friendship || friendship.status !== 'accepted' || isBlocked(req.user.id, otherId))
        return fail(res, 403, 'Direct message unavailable.');
...
app.patch('/api/dms/messages/:messageId', requireAuth, (req, res) => {
    const messageId = parseId(req.params.messageId);
    const parsed = z.object({ content: messageSchema }).safeParse(req.body);
    if (!messageId || !parsed.success)
        return fail(res, 400, 'Invalid message.');
    ...
    if (emojiError)
        return fail(res, 403, emojiError);
    const row = db.prepare('SELECT * FROM direct_messages WHERE id = ? AND sender_id = ?').get(messageId, req.user.id);
    if (!row)
        return fail(res, 404, 'Message not found.');
...
app.delete('/api/dms/messages/:messageId', requireAuth, (req, res) => {
    const messageId = parseId(req.params.messageId);
    const row = messageId ? db.prepare('SELECT * FROM direct_messages WHERE id = ? AND sender_id = ?').get(messageId, req.user.id) : null;
    if (!row)
        return fail(res, 404, 'Message not found.');
...
app.post('/api/dms/:userId/read', requireAuth, (req, res) => {
    const otherId = parseId(req.params.userId);
    if (!otherId)
        return fail(res, 400, 'Invalid user.');
    const last = db.prepare(`SELECT COALESCE(MAX(id), 0) AS id FROM direct_messages WHERE (sender_id = ? AND recipient_id = ?) OR (sender_id = ? AND recipient_id = ?)`).get(req.user.id, otherId, otherId, req.user.id);
...
app.post('/api/dm-groups', requireAuth, (req, res) => {
    ...
    if (!parsed.success)
        return fail(res, 400, 'Invalid group.');
    const maxGroups = premiumMembership(req.user.id) ? numericPremiumBenefit(req.user.id, 'maxDmGroups', 20) : 20;
    const ownedGroups = Number(db.prepare('SELECT COUNT(*) AS c FROM dm_threads WHERE owner_id = ?').get(req.user.id).c);
    if (maxGroups > 0 && ownedGroups >= maxGroups)
        return fail(res, 403, `Limit of ${maxGroups} DM groups reached.`);
    const unique = [...new Set(parsed.data.memberIds)].filter((id) => id !== req.user.id);
    for (const id of unique) {
        const friendship = friendshipStatus(req.user.id, id);
        if (!friendship || friendship.status !== 'accepted' || isBlocked(req.user.id, id))
            return fail(res, 403, 'All members must be your friends.');
    }
    ...
    for (const id of unique)
        io.to(`user:${id}`).emit('dm-groups:refresh');
    return res.status(201).json({ threadId });
});
app.get('/api/dm-groups/:threadId', requireAuth, (req, res) => {
    const threadId = parseId(req.params.threadId);
    if (!threadId || !db.prepare('SELECT 1 FROM dm_thread_members WHERE thread_id = ? AND user_id = ?').get(threadId, req.user.id))
        return fail(res, 404, 'Group not found.');
    const thread = db.prepare('SELECT * FROM dm_threads WHERE id = ?').get(threadId);
    const members = db.prepare('SELECT u.* FROM dm_thread_members m JOIN users u ON u.id = m.user_id WHERE m.thread_id = ? ORDER BY u.display_name').all(threadId).map((u) => publicUser(u));
    ...
    FROM dm_thread_messages m JOIN users u ON u.id = m.author_id WHERE m.thread_id = ? ORDER BY m.id DESC LIMIT 100`).all(threadId).reverse().map((m) => ({
...
app.post('/api/dm-groups/:threadId/messages', requireAuth, (req, res) => {
    const threadId = parseId(req.params.threadId);
    const parsed = z.object({ content: messageSchema }).safeParse(req.body);
    if (!threadId || !parsed.success || !db.prepare('SELECT 1 FROM dm_thread_members WHERE thread_id = ? AND user_id = ?').get(threadId, req.user.id))
        return fail(res, 403, 'Invalid group.');
    const emojiError = validateCustomEmojiContent(req.user.id, parsed.data.content);
    if (emojiError)
        return fail(res, 403, emojiError);
    ...
    const members = db.prepare('SELECT user_id FROM dm_thread_members WHERE thread_id = ?').all(threadId);
    for (const member of members)
        io.to(`user:${member.user_id}`).emit('dm-group:new', message);
    return res.status(201).json({ message });
});
app.post('/api/servers/:serverId/roles', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !hasPermission(req.user.id, serverId, Permission.MANAGE_ROLES))
        return fail(res, 403, 'Permission denied.');
    ...
    const max = Number(db.prepare('SELECT COALESCE(MAX(position), 0) AS p FROM roles WHERE server_id = ?').get(serverId).p);
...
app.patch('/api/roles/:roleId', requireAuth, (req, res) => {
    const roleId = parseId(req.params.roleId);
    const role = roleId ? db.prepare('SELECT * FROM roles WHERE id = ?').get(roleId) : null;
    if (!role || !hasPermission(req.user.id, Number(role.server_id), Permission.MANAGE_ROLES))
        return fail(res, 403, 'Permission denied.');
    ...
    if (!parsed.success)
        return fail(res, 400, 'Invalid role.');
    if (role.is_everyone && parsed.data.name && parsed.data.name !== '@everyone')
        return fail(res, 400, 'The @everyone role cannot be renamed.');
...
app.delete('/api/roles/:roleId', requireAuth, (req, res) => {
    const roleId = parseId(req.params.roleId);
    const role = roleId ? db.prepare('SELECT * FROM roles WHERE id = ?').get(roleId) : null;
    if (!role || role.is_everyone || !hasPermission(req.user.id, Number(role.server_id), Permission.MANAGE_ROLES))
        return fail(res, 403, 'This role cannot be removed.');
    db.prepare('DELETE FROM roles WHERE id = ?').run(roleId);
    audit(Number(role.server_id), req.user.id, 'ROLE_DELETE', 'ROLE', roleId);
    emitServerRefresh(Number(role.server_id));
    return res.status(204).end();
});
app.post('/api/servers/:serverId/roles/reorder', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !hasPermission(req.user.id, serverId, Permission.MANAGE_ROLES))
        return fail(res, 403, 'Permission denied.');
...
app.put('/api/servers/:serverId/members/:userId/roles/:roleId', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId), userId = parseId(req.params.userId), roleId = parseId(req.params.roleId);
    if (!serverId || !userId || !roleId || !hasPermission(req.user.id, serverId, Permission.MANAGE_ROLES))
        return fail(res, 403, 'Permission denied.');
    const role = db.prepare('SELECT * FROM roles WHERE id = ? AND server_id = ? AND is_everyone = 0').get(roleId, serverId);
    if (!role || !isMember(userId, serverId))
        return fail(res, 404, 'Invalid role or member.');
...
app.delete('/api/servers/:serverId/members/:userId/roles/:roleId', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId), userId = parseId(req.params.userId), roleId = parseId(req.params.roleId);
    if (!serverId || !userId || !roleId || !hasPermission(req.user.id, serverId, Permission.MANAGE_ROLES))
        return fail(res, 403, 'Permission denied.');
...
app.patch('/api/servers/:serverId/members/:userId', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId), userId = parseId(req.params.userId);
    if (!serverId || !userId || (!hasPermission(req.user.id, serverId, Permission.MANAGE_SERVER) && req.user.id !== userId))
        return fail(res, 403, 'Permission denied.');
...
app.post('/api/servers/:serverId/members/:userId/kick', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId), userId = parseId(req.params.userId);
    if (!serverId || !userId || !hasPermission(req.user.id, serverId, Permission.KICK_MEMBERS) || isServerOwner(userId, serverId))
        return fail(res, 403, 'Permission denied.');
...
app.post('/api/servers/:serverId/members/:userId/ban', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId), userId = parseId(req.params.userId);
    if (!serverId || !userId || !hasPermission(req.user.id, serverId, Permission.BAN_MEMBERS) || isServerOwner(userId, serverId))
        return fail(res, 403, 'Permission denied.');
...
app.get('/api/servers/:serverId/bans', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !hasPermission(req.user.id, serverId, Permission.BAN_MEMBERS))
        return fail(res, 403, 'Permission denied.');
    const bans = db.prepare(`SELECT b.*, u.username, u.display_name, u.avatar_path FROM server_bans b JOIN users u ON u.id = b.user_id WHERE b.server_id = ? ORDER BY b.created_at DESC`).all(serverId).map((b) => ({
...
app.delete('/api/servers/:serverId/bans/:userId', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId), userId = parseId(req.params.userId);
    if (!serverId || !userId || !hasPermission(req.user.id, serverId, Permission.BAN_MEMBERS))
        return fail(res, 403, 'Permission denied.');
...
app.get('/api/servers/:serverId/audit', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !hasPermission(req.user.id, serverId, Permission.MANAGE_SERVER))
        return fail(res, 403, 'Permission denied.');
    const logs = db.prepare(`SELECT a.*, u.username, u.display_name, u.avatar_path FROM audit_logs a LEFT JOIN users u ON u.id = a.actor_id WHERE a.server_id = ? ORDER BY a.id DESC LIMIT 200`).all(serverId).map((a) => ({
...
app.get('/api/servers/:serverId/notifications', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !isMember(req.user.id, serverId))
        return fail(res, 403, 'Access denied.');
    const row = db.prepare('SELECT level, muted FROM notification_settings WHERE user_id = ? AND server_id = ?').get(req.user.id, serverId);
    return res.json({ level: row?.level ?? 'MENTIONS', muted: Boolean(row?.muted) });
});
app.put('/api/servers/:serverId/notifications', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !isMember(req.user.id, serverId))
        return fail(res, 403, 'Access denied.');
    const parsed = z.object({ level: z.enum(['ALL', 'MENTIONS', 'NONE']), muted: z.boolean() }).safeParse(req.body);
    if (!parsed.success)
        return fail(res, 400, 'Invalid preference.');
...
app.post('/api/servers/:serverId/voice/:userId', requireAuth, async (req, res) => {
    const serverId = parseId(req.params.serverId), userId = parseId(req.params.userId);
    if (!serverId || !userId || !hasPermission(req.user.id, serverId, Permission.MANAGE_SERVER))
        return fail(res, 403, 'Permission denied.');
    ...
    for (const state of targets) {
        ...
        if (parsed.data.action === 'disconnect') {
            ...
            if (!target)
                return fail(res, 400, 'Invalid destination channel.');
            ...
            if (parsed.data.action === 'deafen')
                state.serverDeafened = true;
            if (parsed.data.action === 'undeafen')
                state.serverDeafened = false;
...
app.get('/api/servers/:serverId/export', requireAuth, (req, res) => {
    const serverId = parseId(req.params.serverId);
    if (!serverId || !isServerOwner(req.user.id, serverId))
        return fail(res, 403, 'Only the owner can export this server.');
    const serverRow = db.prepare('SELECT id, name, description FROM servers WHERE id = ?').get(serverId);
    const channels = db.prepare('SELECT name, kind, parent_id, position, topic, user_limit, bitrate, id FROM channels WHERE server_id = ? ORDER BY position').all(serverId);
    const roles = db.prepare('SELECT name, color, permissions, position, is_everyone FROM roles WHERE server_id = ? ORDER BY position').all(serverId);
...
app.post('/api/servers/import', requireAuth, (req, res) => {
    ...
    if (!parsed.success)
        return fail(res, 400, 'Invalid export file.');
    const inviteCode = crypto.randomBytes(6).toString('base64url');
    const serverId = db.transaction(() => {
        ...
        for (const item of parsed.data.channels)
            if (item.parent_id && idMap.has(Number(item.id)) && idMap.has(Number(item.parent_id)))
                db.prepare('UPDATE channels SET parent_id = ? WHERE id = ?').run(idMap.get(Number(item.parent_id)), idMap.get(Number(item.id)));
...
app.get('/api/admin/stats', requireAuth, requireInstanceAdmin, (_req, res) => {
    const now = Date.now();
    const counts = {
        users: Number(db.prepare('SELECT COUNT(*) AS c FROM users').get().c),
        disabledUsers: Number(db.prepare('SELECT COUNT(*) AS c FROM users WHERE disabled = 1').get().c),
        servers: Number(db.prepare('SELECT COUNT(*) AS c FROM servers').get().c),
        messages: Number(db.prepare('SELECT (SELECT COUNT(*) FROM messages) + (SELECT COUNT(*) FROM direct_messages) + (SELECT COUNT(*) FROM dm_thread_messages) AS c').get().c),
        online: onlineUsers.size,
        sessions: Number(db.prepare('SELECT COUNT(*) AS c FROM sessions WHERE expires_at > ?').get(now).c),
        activeBans: Number(db.prepare('SELECT COUNT(*) AS c FROM instance_bans WHERE revoked_at IS NULL AND (expires_at IS NULL OR expires_at > ?)').get(now).c),
    };
    let uploadBytes = 0;
    for (const file of fs.readdirSync(uploadDir)) {
        try {
            uploadBytes += fs.statSync(path.join(uploadDir, file)).size;
        }
        catch { }
    }
...
app.get('/api/admin/users/:userId', requireAuth, requireInstanceAdmin, (req, res) => {
  ...
  `).get(Date.now(), userId);
    if (!row)
        return fail(res, 404, 'User not found.');
    const sessions = db.prepare('SELECT token_hash, created_at, expires_at FROM sessions WHERE user_id = ? ORDER BY created_at DESC').all(userId).map((item) => ({
  ...
  `).all(userId).map((item) => ({ id: Number(item.id), name: item.name, iconPath: item.icon_path ?? null, joinedAt: item.joined_at }));
  ...
  `).all(userId).map((item) => ({ id: Number(item.id), note: item.note, createdAt: item.created_at, authorId: item.author_id == null ? null : Number(item.author_id), authorName: item.author_name ?? 'Former administrator' }));
...
app.put('/api/admin/users/:userId/disabled', requireAuth, requireInstanceAdmin, async (req, res) => {
    const userId = parseId(req.params.userId);
    const parsed = z.object({ disabled: z.boolean(), reason: z.string().trim().max(500).optional() }).safeParse(req.body);
    if (!userId || !parsed.success || userId === req.user.id)
        return fail(res, 400, 'Invalid operation.');
    const target = db.prepare('SELECT id, is_instance_admin FROM users WHERE id = ?').get(userId);
    if (!target)
        return fail(res, 404, 'User not found.');
    if (target.is_instance_admin)
        return fail(res, 409, 'Remove administrator access before disabling this account.');
...
app.put('/api/admin/users/:userId/admin', requireAuth, requireInstanceAdmin, (req, res) => {
    const userId = parseId(req.params.userId);
    const parsed = z.object({ isAdmin: z.boolean() }).safeParse(req.body);
    if (!userId || !parsed.success)
        return fail(res, 400, 'Invalid operation.');
    const target = db.prepare('SELECT id, is_instance_admin, disabled FROM users WHERE id = ?').get(userId);
    if (!target)
        return fail(res, 404, 'User not found.');
    if (parsed.data.isAdmin && (Boolean(target.disabled) || activeInstanceBan(userId)))
        return fail(res, 409, 'Reactivate and unban the user before granting administrator access.');
    if (!parsed.data.isAdmin && target.is_instance_admin) {
        const adminCount = Number(db.prepare('SELECT COUNT(*) AS c FROM users WHERE is_instance_admin = 1').get().c);
        if (adminCount <= 1)
            return fail(res, 409, 'The instance must keep at least one administrator.');
    }
    db.transaction(() => {
        ...
        if (adminBadge) {
            if (parsed.data.isAdmin)
                db.prepare('INSERT OR IGNORE INTO user_badges (user_id, badge_id, assigned_by) VALUES (?, ?, ?)').run(userId, Number(adminBadge.id), req.user.id);
            else
                db.prepare('DELETE FROM user_badges WHERE user_id = ? AND badge_id = ?').run(userId, Number(adminBadge.id));
        }
    })();
...
app.post('/api/admin/users/:userId/logout', requireAuth, requireInstanceAdmin, async (req, res) => {
    const userId = parseId(req.params.userId);
    if (!userId)
        return fail(res, 400, 'Invalid user.');
    destroyAllSessions(userId);
...
app.delete('/api/admin/sessions/:sessionId', requireAuth, requireInstanceAdmin, (req, res) => {
    const sessionId = String(req.params.sessionId ?? '');
    if (!/^[a-f0-9]{64}$/i.test(sessionId))
        return fail(res, 400, 'Invalid session.');
    const row = db.prepare('SELECT user_id FROM sessions WHERE token_hash = ?').get(sessionId);
    if (!row)
        return res.status(204).end();
...
app.delete('/api/admin/users/:userId', requireAuth, requireInstanceAdmin, async (req, res) => {
    const userId = parseId(req.params.userId);
    const parsed = z.object({ confirmation: z.string().trim(), reason: z.string().trim().min(1).max(500) }).safeParse(req.body);
    if (!userId || !parsed.success || userId === req.user.id)
        return fail(res, 400, 'Invalid operation.');
    const target = db.prepare('SELECT id, username, is_instance_admin, avatar_path, banner_path, avatar_decoration_path, profile_background_path, custom_join_sound_path FROM users WHERE id = ?').get(userId);
    if (!target)
        return fail(res, 404, 'User not found.');
    if (target.is_instance_admin)
        return fail(res, 409, 'Remove administrator access before deleting this account.');
    if (parsed.data.confirmation.toLowerCase() !== String(target.username).toLowerCase())
        return fail(res, 400, 'Digite o username exato para confirmar.');
    const memberships = db.prepare('SELECT server_id FROM server_members WHERE user_id = ?').all(userId);
    const ownedServers = db.prepare('SELECT id, name, icon_path, banner_path FROM servers WHERE owner_id = ?').all(userId);
    const ownedServerIds = new Set(ownedServers.map((server) => Number(server.id)));
    const uploaded = new Set();
    for (const value of [target.avatar_path, target.banner_path, target.avatar_decoration_path, target.profile_background_path, target.custom_join_sound_path])
        if (typeof value === 'string' && value.startsWith('/uploads/'))
            uploaded.add(value);
    const serverProfileAvatars = db.prepare('SELECT avatar_path FROM server_profiles WHERE user_id = ?').all(userId);
    for (const item of serverProfileAvatars)
        if (item.avatar_path?.startsWith('/uploads/'))
            uploaded.add(item.avatar_path);
    const premiumEmojiImages = db.prepare('SELECT image_path FROM premium_emojis WHERE user_id = ?').all(userId);
    for (const item of premiumEmojiImages)
        if (item.image_path?.startsWith('/uploads/'))
            uploaded.add(item.image_path);
    const authoredAttachments = db.prepare('SELECT ma.path FROM message_attachments ma JOIN messages m ON m.id = ma.message_id WHERE m.author_id = ?').all(userId);
    for (const item of authoredAttachments)
        if (item.path?.startsWith('/uploads/'))
            uploaded.add(item.path);
    for (const server of ownedServers) {
        for (const value of [server.icon_path, server.banner_path])
            if (typeof value === 'string' && value.startsWith('/uploads/'))
                uploaded.add(value);
        const serverAttachments = db.prepare('SELECT ma.path FROM message_attachments ma JOIN messages m ON m.id = ma.message_id JOIN channels c ON c.id = m.channel_id WHERE c.server_id = ?').all(Number(server.id));
        for (const item of serverAttachments)
            if (item.path?.startsWith('/uploads/'))
                uploaded.add(item.path);
        const memberIds = db.prepare('SELECT user_id FROM server_members WHERE server_id = ?').all(Number(server.id));
        for (const member of memberIds)
            io.to(`user:${Number(member.user_id)}`).emit('server:deleted', { serverId: Number(server.id), reason: `Server removed because the owner account was deleted. ${parsed.data.reason}` });
    }
    ...
    for (const membership of memberships)
        if (!ownedServerIds.has(Number(membership.server_id)))
            emitServerRefresh(Number(membership.server_id));
    io.emit('friends:refresh');
    for (const filePath of uploaded)
        fs.rm(path.join(uploadDir, path.basename(filePath)), { force: true }, () => undefined);
    return res.status(204).end();
});
app.post('/api/admin/users/:userId/notes', requireAuth, requireInstanceAdmin, (req, res) => {
    const userId = parseId(req.params.userId);
    const parsed = z.object({ note: z.string().trim().min(1).max(1000) }).safeParse(req.body);
    if (!userId || !parsed.success || !db.prepare('SELECT 1 FROM users WHERE id = ?').get(userId))
...
app.delete('/api/admin/notes/:noteId', requireAuth, requireInstanceAdmin, (req, res) => {
    const noteId = parseId(req.params.noteId);
    if (!noteId)
        return fail(res, 400, 'Invalid note.');
    const note = db.prepare('SELECT user_id FROM admin_user_notes WHERE id = ?').get(noteId);
    if (!note)
        return res.status(204).end();
...
app.post('/api/admin/users/:userId/ban', requireAuth, requireInstanceAdmin, async (req, res) => {
    const userId = parseId(req.params.userId);
    ...
    if (!userId || !parsed.success || userId === req.user.id)
        return fail(res, 400, 'Invalid ban.');
    const target = db.prepare('SELECT id, is_instance_admin FROM users WHERE id = ?').get(userId);
    if (!target)
        return fail(res, 404, 'User not found.');
    if (target.is_instance_admin)
        return fail(res, 409, 'Remove administrator access before banning this account.');
    if (activeInstanceBan(userId))
        return fail(res, 409, 'This user already has an active ban.');
...
app.post('/api/admin/bans/:banId/revoke', requireAuth, requireInstanceAdmin, (req, res) => {
    const banId = parseId(req.params.banId);
    const parsed = z.object({ reason: z.string().trim().max(500).default('') }).safeParse(req.body);
    if (!banId || !parsed.success)
        return fail(res, 400, 'Invalid ban.');
    const ban = db.prepare('SELECT id, user_id, revoked_at FROM instance_bans WHERE id = ?').get(banId);
    if (!ban)
        return fail(res, 404, 'Ban not found.');
    if (ban.revoked_at != null)
        return fail(res, 409, 'This ban has already been revoked.');
...
app.post('/api/admin/badges', requireAuth, requireInstanceAdmin, badgeUpload.single('image'), (req, res) => {
    const parsed = z.object({ name: z.string().trim().min(1).max(32) }).safeParse({ name: req.body.name });
    if (!parsed.success || !req.file) {
        if (req.file)
            removeUploaded([req.file]);
        return fail(res, 400, 'Enter a name and upload an image for the badge.');
    }
    if (Number(db.prepare('SELECT COUNT(*) AS count FROM badges').get().count) >= 100) {
        removeUploaded([req.file]);
        return fail(res, 409, 'Limit of 100 badges reached.');
    }
    try {
        const position = Number(db.prepare('SELECT COALESCE(MAX(position), 0) AS max FROM badges').get().max) + 1;
...
app.put('/api/admin/badges/:badgeId', requireAuth, requireInstanceAdmin, badgeUpload.single('image'), (req, res) => {
    const badgeId = parseId(req.params.badgeId);
    const parsed = z.object({ name: z.string().trim().min(1).max(32) }).safeParse({ name: req.body.name });
    if (!badgeId || !parsed.success) {
        if (req.file)
            removeUploaded([req.file]);
        return fail(res, 400, 'Invalid badge.');
    }
    const current = db.prepare('SELECT * FROM badges WHERE id = ?').get(badgeId);
    if (!current) {
        if (req.file)
            removeUploaded([req.file]);
        return fail(res, 404, 'Badge not found.');
    }
    const nextImage = req.file ? `/uploads/${req.file.filename}` : current.image_path;
    if (!nextImage) {
        if (req.file)
            removeUploaded([req.file]);
        return fail(res, 400, 'A badge must have an image.');
    }
    try {
    ...
    if (req.file && current.image_path?.startsWith('/uploads/'))
        fs.rm(path.join(uploadDir, path.basename(current.image_path)), { force: true }, () => undefined);
...
app.post('/api/admin/badges/reorder', requireAuth, requireInstanceAdmin, (req, res) => {
...
app.delete('/api/admin/badges/:badgeId', requireAuth, requireInstanceAdmin, (req, res) => {
    const badgeId = parseId(req.params.badgeId);
    if (!badgeId)
        return fail(res, 400, 'Invalid badge.');
    const badge = db.prepare('SELECT name, image_path FROM badges WHERE id = ?').get(badgeId);
    if (!badge)
        return res.status(204).end();
    db.prepare('DELETE FROM badges WHERE id = ?').run(badgeId);
    if (badge.image_path?.startsWith('/uploads/'))
        fs.rm(path.join(uploadDir, path.basename(badge.image_path)), { force: true }, () => undefined);
    instanceAudit(req.user.id, 'BADGE_DELETE', 'BADGE', badgeId, { name: badge.name });
    io.emit('badges:refresh');
    return res.status(204).end();
});
app.put('/api/admin/users/:userId/badges', requireAuth, requireInstanceAdmin, (req, res) => {
    const userId = parseId(req.params.userId);
    const parsed = z.object({ badgeIds: z.array(z.number().int().positive()).max(100) }).safeParse(req.body);
    if (!userId || !parsed.success || !db.prepare('SELECT 1 FROM users WHERE id = ?').get(userId))
        return fail(res, 400, 'Invalid user or badges.');
    const uniqueIds = [...new Set(parsed.data.badgeIds)];
    if (uniqueIds.length) {
        const placeholders = uniqueIds.map(() => '?').join(',');
        const existing = Number(db.prepare(`SELECT COUNT(*) AS count FROM badges WHERE id IN (${placeholders})`).get(...uniqueIds).count);
        if (existing !== uniqueIds.length)
            return fail(res, 400, 'One of the badges does not exist.');
    }
    db.transaction(() => {
        ...
        for (const badgeId of uniqueIds)
            insert.run(userId, badgeId, req.user.id);
    })();
...
app.delete('/api/admin/servers/:serverId', requireAuth, requireInstanceAdmin, (req, res) => {
    const serverId = parseId(req.params.serverId);
    const parsed = z.object({ reason: z.string().trim().min(1).max(500) }).safeParse(req.body);
    if (!serverId || !parsed.success)
        return fail(res, 400, 'Invalid operation.');
    const serverRow = db.prepare('SELECT id, name, icon_path, banner_path FROM servers WHERE id = ?').get(serverId);
    if (!serverRow)
        return fail(res, 404, 'Server not found.');
    const attachmentPaths = db.prepare('SELECT ma.path FROM message_attachments ma JOIN messages m ON m.id = ma.message_id JOIN channels c ON c.id = m.channel_id WHERE c.server_id = ?').all(serverId).map((row) => String(row.path));
    const memberIds = db.prepare('SELECT user_id FROM server_members WHERE server_id = ?').all(serverId);
    io.to(`server:${serverId}`).emit('server:deleted', { serverId, reason: parsed.data.reason });
    for (const member of memberIds)
        io.to(`user:${Number(member.user_id)}`).emit('server:deleted', { serverId, reason: parsed.data.reason });
    instanceAudit(req.user.id, 'SERVER_DELETE', 'SERVER', serverId, { name: serverRow.name, reason: parsed.data.reason });
    db.prepare('DELETE FROM servers WHERE id = ?').run(serverId);
    const files = [serverRow.icon_path, serverRow.banner_path, ...attachmentPaths].filter((value) => typeof value === 'string' && value.startsWith('/uploads/'));
...
app.get('/api/admin/premium', requireAuth, requireInstanceAdmin, (_req, res) => {
    ...
    FROM user_premium p JOIN users u ON u.id = p.user_id ORDER BY CASE WHEN p.expires_at IS NULL THEN 0 ELSE 1 END, p.expires_at DESC`).all().map((row) => ({
...
app.put('/api/admin/premium', requireAuth, requireInstanceAdmin, (req, res) => {
    const parsed = z.object({
    ...
    if (!parsed.success)
        return fail(res, 400, 'Invalid premium plan configuration.');
...
app.post('/api/admin/premium/icon', requireAuth, requireInstanceAdmin, premiumImageUpload.single('icon'), (req, res) => {
    if (!req.file)
        return fail(res, 400, 'Select PNG, JPEG, WebP, or GIF.');
...
app.put('/api/admin/users/:userId/premium', requireAuth, requireInstanceAdmin, (req, res) => {
    const userId = parseId(req.params.userId);
    if (!userId)
        return fail(res, 400, 'Invalid user.');
    ...
    if (!active && !inactive)
        return fail(res, 400, 'Invalid premium plan state.');
...
app.put('/api/admin/monetization', requireAuth, requireInstanceAdmin, (req, res) => {
    const parsed = z.object({
        enabled: z.boolean(),
    ...
    if (!parsed.success)
        return fail(res, 400, 'Invalid monetization configuration. Use only HTTP or HTTPS links.');
...
app.get('/api/admin/monetization/codes', requireAuth, requireInstanceAdmin, (_req, res) => {
    const codes = db.prepare(`SELECT id, code_prefix, duration_minutes, max_uses, use_count, disabled, created_at FROM premium_redeem_codes ORDER BY id DESC LIMIT 250`).all().map((row) => ({
...
app.post('/api/admin/monetization/codes', requireAuth, requireInstanceAdmin, (req, res) => {
    const parsed = z.object({ durationDays: z.number().int().min(0).max(3650), maxUses: z.number().int().min(1).max(1000) }).safeParse(req.body);
    if (!parsed.success)
        return fail(res, 400, 'Invalid redemption-code configuration.');
...
app.delete('/api/admin/monetization/codes/:id', requireAuth, requireInstanceAdmin, (req, res) => {
    const id = parseId(req.params.id);
    if (!id)
        return fail(res, 400, 'Invalid code.');
    const result = db.prepare('UPDATE premium_redeem_codes SET disabled = 1 WHERE id = ?').run(id);
    if (result.changes === 0)
        return fail(res, 404, 'Redemption code not found.');
...
app.put('/api/admin/settings', requireAuth, requireInstanceAdmin, (req, res) => {
    const parsed = z.object({
...
app.post('/api/admin/logo', requireAuth, requireInstanceAdmin, imageUpload.single('logo'), (req, res) => {
    if (!req.file)
        return fail(res, 400, 'Select an image.');
    ...
    if (previous.startsWith('/uploads/'))
        fs.rm(path.join(uploadDir, path.basename(previous)), { force: true }, () => undefined);
    instanceAudit(req.user.id, 'INSTANCE_LOGO_UPDATE', 'INSTANCE');
    return res.json({ logo: nextPath });
});
app.post('/api/admin/backup', requireAuth, requireInstanceAdmin, (req, res) => {
...
app.get('/api/admin/backups/:name/download', requireAuth, requireInstanceAdmin, (req, res) => {
    ...
    if (!/^[a-zA-Z0-9._-]+\.db$/.test(name))
        return fail(res, 400, 'Invalid backup.');
    const target = path.join(backupDir, name);
    if (!fs.existsSync(target))
        return fail(res, 404, 'Backup not found.');
    return res.download(target, name);
});
app.delete('/api/admin/backups/:name', requireAuth, requireInstanceAdmin, (req, res) => {
    const name = path.basename(String(req.params.name));
    if (!/^[a-zA-Z0-9._-]+\.db$/.test(name))
        return fail(res, 400, 'Invalid backup.');
    const target = path.join(backupDir, name);
...
io.on('connection', (socket) => {
    ...
    const membershipRooms = db.prepare('SELECT server_id FROM server_members WHERE user_id = ?').all(user.id);
    for (const membership of membershipRooms)
        socket.join(`notify-server:${Number(membership.server_id)}`);
    ...
    socket.on('latency:ping', (ack) => ack?.(Date.now()));
    socket.on('server:subscribe', (serverIdRaw) => {
        const serverId = parseId(serverIdRaw);
        if (serverId && isMember(user.id, serverId)) {
            socket.join(`server:${serverId}`);
            socket.join(`notify-server:${serverId}`);
        }
    });
    ...
    socket.on('channel:subscribe', (channelIdRaw) => {
        ...
        if (channelId && canAccessChannel(user.id, channelId))
    ...
    socket.on('typing:channel', (payload) => {
        const channelId = parseId(payload?.channelId);
        const currentUser = userById(user.id) ?? user;
        socket.data.user = currentUser;
        if (channelId && canAccessChannel(currentUser.id, channelId))
            socket.to(`channel:${channelId}`).emit('typing:channel', { channelId, user: currentUser });
    });
    socket.on('typing:dm', (payload) => {
        ...
        if (friendship?.status === 'accepted' && !isBlocked(currentUser.id, targetId))
            io.to(`user:${targetId}`).emit('typing:dm', { user: currentUser });
    });
    socket.on('voice:join', async (channelIdRaw, acknowledge) => {
        const currentUser = userById(user.id) ?? user;
        socket.data.user = currentUser;
        ...
        if (!channelId || !canAccessChannel(currentUser.id, channelId) || !hasChannelPermission(currentUser.id, channelId, Permission.CONNECT))
            return acknowledge?.({ error: 'Invalid voice channel or permission denied.' });
        const channel = db.prepare("SELECT * FROM channels WHERE id = ? AND kind = 'VOICE'").get(channelId);
        if (!channel)
            return acknowledge?.({ error: 'Invalid voice channel.' });
        ...
        if (Number(channel.user_limit) > 0 && existingSockets.length >= Number(channel.user_limit) && !hasPriorityVoice && !hasPermission(currentUser.id, Number(channel.server_id), Permission.MANAGE_SERVER))
            return acknowledge?.({ error: 'Voice channel is full.' });
        leaveVoiceSocket(socket);
        const peers = existingSockets.map((peer) => {
            const peerUser = userById(Number(peer.data.user?.id)) ?? peer.data.user;
            if (peerUser)
                peer.data.user = peerUser;
            return { socketId: peer.id, user: peerUser, state: voiceStates.get(peer.id) };
        });
        socket.join(room);
        socket.data.voiceRoom = room;
        socket.data.voiceChannelId = channelId;
        const state = { socketId: socket.id, userId: currentUser.id, user: currentUser, channelId, serverId: Number(channel.server_id), selfMuted: false, selfDeafened: false, serverMuted: false, serverDeafened: false, cameraOn: false, screenOn: false };
    ...
    socket.on('voice:state-update', (payload) => {
        ...
        if (!state)
            return;
        if (typeof payload?.selfMuted === 'boolean')
            state.selfMuted = payload.selfMuted;
        if (typeof payload?.selfDeafened === 'boolean')
            state.selfDeafened = payload.selfDeafened;
        if (typeof payload?.cameraOn === 'boolean')
            state.cameraOn = payload.cameraOn;
        if (typeof payload?.screenOn === 'boolean')
            state.screenOn = payload.screenOn;
    ...
    socket.on('webrtc:signal', (payload) => {
        ...
        if (!target || target.data.voiceRoom !== room)
            return;
        const currentUser = userById(user.id) ?? user;
        socket.data.user = currentUser;
...
app.use((error, _req, res, _next) => {
    ...
    if (error instanceof multer.MulterError)
        return fail(res, 400, error.code === 'LIMIT_FILE_SIZE' ? 'File is too large.' : 'Upload failed.');
...
setInterval(() => {
    const now = Date.now();
    const expired = db.prepare(`SELECT user_id FROM user_premium WHERE expires_at IS NOT NULL AND expires_at > ? AND expires_at <= ?`).all(lastPremiumExpirySweep, now);
...
server.listen(port, host, () => {
    ...
    if (host === '0.0.0.0')
        console.log(`Local network: open http://YOUR-IP:${port} on another device.`);
});
